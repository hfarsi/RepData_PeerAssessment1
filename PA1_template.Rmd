...
There is a clear pattern in daily activities. Activities pick up in the morning and fluctuates throughout the day with the maximum peak value of `r peak`, which occurs in interval `r interval`, around `r round(as.numeric(interval)/12,0)` AM.        
...
The original data set has `r nrow(miss_obs)` NAs in the steps variable, which belong to `r length(uniq_missing_date)` days. That is, there isn't a single observation for either of the missing days, perhaps the subject was not wearing the device on those days. Most of the missing data happens on wednesday of the week.  
